...
const cart = JSON.parse(localStorage.getItem(`gloDelivery_${login}`)) || []; // Корзина, будет сохраняться в LS
...
function downloadCart() {
  if (localStorage.getItem(`gloDelivery_${login}`)) { // если в LS что-то есть с этим логином
...
function authorized() {
  ...
  function logOut() {
    login = null;
    cart.length = 0; // Очистили корзину, чтобы не было видно корзины после предыдущего юзера
...
function addToCart(event) {
  ...
  if (buttonAddToCart) { // если попали на кнопку "В корзину"
    const card = target.closest('.card');
    const title = card.querySelector('.card-title-reg').textContent; // Получили название блюда
    const cost = card.querySelector('.card-price').textContent; // Получили цену блюда
